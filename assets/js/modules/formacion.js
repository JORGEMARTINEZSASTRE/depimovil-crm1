...
async function saveEvaluacionTecnica(){
  ...
  if(aprobada && esHabilitante){
  ...
  }else if(aprobada && (esHIFU || esLaser || esNdYAG || esExilis || esEmsculpt || esHydrafacial || esSoprano || esBronceado || esAparatologia || esMasajes || esCavitacion || esSkincare || esCriolipolisis || esAtencion || esBioseguridad || esRF)){
    ...
    const cert = esHIFU ? 'HIFU' : esNdYAG ? 'Nd:YAG' : esExilis ? 'Exilis Elite' : esEmsculpt ? 'Emsculpt' : esHydrafacial ? 'HydraFacial' : esSoprano ? 'Soprano Titanium ICE' : esBronceado ? 'Bronceado Orgánico' : esAparatologia ? 'Aparatología Estética' : esMasajes ? 'Masajes y Drenaje Linfático' : esCavitacion ? 'Cavitación Ultrasónica' : esSkincare ? 'Skincare y Cuidado de la Piel' : esCriolipolisis ? 'Criolipólisis' : esAtencion ? 'Atención al Cliente y Ventas en Estética' : esBioseguridad ? 'Bioseguridad e Higiene en Estética' : esRF ? 'Radiofrecuencia Corporal y Facial' : 'Depilación Láser';
