...
router.post('/:id/habilitaciones', auth, requireRole('superadmin', 'operaciones'), async (req, res) => {
  const { categoria, estado, fecha_habilitacion, obs } = req.body;
  if (!categoria) return res.status(400).json({ error: 'Categoría es obligatoria' });
  try {
    const { rows } = await pool.query(
      `INSERT INTO habilitaciones (operadora_id, categoria, estado, fecha_habilitacion, obs)
       ...
       RETURNING *`,
      [req.params.id, categoria, estado || 'activa', fecha_habilitacion || null, obs || null]
    );
